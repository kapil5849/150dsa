...
const Pattern = () => {
  const questions = [
    ...
    },
    {
      ...
      detailsLink: "../stack-of-plates",
    },
    {
      title: 'Queue via Stacks',
      description: 'Implement a MyQueue class which implements a queue using two stacks.',
      detailsLink: '../queue-via-stacks'
    },
    {
      title: 'Sort Stack',
      description: 'Write a program to sort a stack such that the smallest items are on the top. You can use an additional temporary stack, but you may not copy the elements into any other data structure (such as an array). The stack supports the following operations: push, pop, peek, and is Empty.',
      detailsLink: '../sort-stack'
    },
    {
      title: 'Animal Shelter',
      description: 'An animal shelter, which holds only dogs and cats, operates on a strictly"first in, first out" basis. People must adopt either the "oldest" (based on arrival time) of all animals at the shelter, or they can select whether they would prefer a dog or a cat (and will receive the oldest animal of that type). They cannot select which specific animal they would like. Create the data structures to maintain this system and implement operations such as enqueue, dequeueAny, dequeueDog, and dequeueCat. You may use the built-in Linked list data structure.',
      detailsLink: '../animal-shelter'
    }
  ];
  return (
    <div className='stack-container'>
      <ArticleHeading> Stacks and Queues </ArticleHeading>
      <div className='stack-content'>
        ...
        <Heading level={1}>🌟 Stacks</Heading>
        <BulletPoints
          items={[
            ...
            {
              item: 'Common operations on a stack include:',
              subItems: ['Push: Adds an element to the top of the stack.', 'Pop: Removes and returns the top element of the stack.',
                'Peek: Returns the top element of the stack without removing it.']
            },
            {
              item: 'Stacks are used in various scenarios, such as:',
              subItems: ['Function call stack: Keeps track of function calls and their execution context.'
        ...
        <BulletPoints
          items={[
            ...
            {
              item: 'Common operations on a queue include:',
              subItems: ['Enqueue: Adds an element to the rear of the queue.', 'Dequeue: Removes and returns the front element of the queue.', 'Peek: Returns the front element of the queue without removing it.']
            },
            {
              item: 'Queues are used in various scenarios, such as:',
              subItems: ['Process scheduling: Managing tasks in an operating system, where the first task in is the next to be executed.', 'Print spooling: Managing print jobs in a queue for a printer.', 'Message queues: Handling inter-process communication in distributed systems.', 'Breadth-first search: Traversing graphs level by level.']
            },
          ]}
        />
        <Example
          code="NOTE : Both stacks and queues are abstract data types that can be implemented using arrays or linked lists. There are also specialized variations of these structures, such as priority queues and double-ended queues (deques), which provide additional functionalities based on specific requirements."
        />
        <Heading level={3}>🌟 Here are the operations for stacks and queues in C++ along with comments explaining each operation:</Heading>
        <Heading level={3}>⏩ Stack Operations:</Heading>
        <CodeSnippet
          code={`
#include <stack>
...
return 0;
        ...
        `}
        />
        <Heading level={3}>⏩ Queues Operations: </Heading>
        <CodeSnippet
          code={`
          #include <queue>
        ...
        />
        <Example
          code="Note: Make sure to include the relevant header file <stack> for stacks and <queue> for queues before using these operations in your C++ program."
        />
        <Heading level={4}>The statement unlike an array, a stack does not offer constant-time access to the ith item means that in a stack, you cannot directly access or retrieve an element at a specific position (ith index) like you can in an array. In a stack, you can only access the topmost element.</Heading>
        ...
        `} />
        <Heading level={3}>However, if we try to access an element at a specific position (ith index) like myStack[1], it will result in a compilation error because stacks do not provide direct access to elements by index. Stacks are designed to allow constant-time adds and removes from the top, but not random access to elements in the middle or at specific positions.</Heading>
        <Heading level={3}>In the above example, we create a stack myStack and push three elements onto it (10, 20, and 30). We can access the topmost element using the top() function, which gives us direct access to the element at the top of the stack (30 in this case).</Heading>
        <Example code="Let's explain the second point: it does allow constant-time adds and removes, as it doesn't require shifting elements around." />
        <Heading level={3}>In a stack, elements are added and removed from only one end, called the top of the stack. When you add an element, it is placed on top of the stack, and when you remove an element, you remove the topmost element.</Heading>
        <Heading level={3}>This characteristic of stacks allows for constant-time adds and removes. It means that regardless of the number of elements in the stack, adding or removing an element takes the same amount of time, as the operation is always performed at the top of the stack.</Heading>
        <Example code="This characteristic of stacks allows for constant-time adds and removes. It means that regardless of the number of elements in the stack, adding or removing an element takes the same amount of time, as the operation is always performed at the top of the stack." />
        ...
        `} />
        <BulletPoints
          items={['In the above example, we create a stack myStack and add three elements to it (10, 20, and 30). When we push elements onto the stack, they are added to the top of the stack. The top() function allows us to access the topmost element directly, which in this case is 30.',
        ...
        />
        <Example
          code="The important point to note here is that adding and removing elements from the stack is a constant-time operation because it doesn't require shifting or rearranging the elements in the stack. It simply adds or removes an element from the top, making it an efficient and constant-time process."
        />
        <Heading level={1}>🌟 What is constant Time?</Heading>
        <Heading level={3}>Constant Time</Heading>
        <BulletPoints items={[
          ...
          'In the context of the previous explanation about stacks, when we say that adding and removing elements from a stack is a constant - time operation, it means that regardless of the number of elements in the stack, the time it takes to add or remove an element remains the same.It does not increase as the stack grows larger.',
        ...
        <Example code="Here's an example of implementing a stack and a queue in C++ using all the common methods:" />
        <CodeSnippet code={[`
        #include <iostream>
        #include <stack>
        #include <queue>
        ...
        `]} />
        <Heading level={3}>This code demonstrates the basic operations for both a stack and a queue. It includes pushing elements onto the stack/queue, checking if it's empty, accessing the top/front/back elements, popping elements, and getting the size of the stack/queue.</Heading>
        <Example code="Here's the revised code with comments explaining each class and significant sections:" />
        <CodeSnippet code={[`
        #include <iostream>
        ...
        `]} />
        <Example code="Here's the code implementing the Stack using a linked list and the Queue using an array with comments explaining each section:" />
        <CodeSnippet
          code={[`
          #include <iostream>
        ...
        `]}
        />
        <Example code="There are several ways to implement a stack using other data structures. Here are a few common approaches:" />
        <BulletPoints
          items={[
        ...
        />
        <Example code="Here are several possible methods to implement a stack using different data structures:" />
        <BulletPoints
          items={[
        ...
        />
        <Heading level={3}>These are some of the commonly used methods to implement a stack using different data structures. Each approach has its own advantages and considerations, so the choice depends on factors such as performance requirements, memory usage, and programming language capabilities.</Heading>
        <Example code="Here are some possible methods to implement a queue using different data structures:" />
        <BulletPoints
          items={[
            {
              item: 'Array-based implementation:',
              subItems: ['Use an array to store the elements of the queue. Maintain two pointers, front and rear, to keep track of the start and end of the queue. When an element is enqueued, it is added to the rear of the queue, and when an element is dequeued, it is removed from the front of the queue. This implementation can be done using a fixed-size array or a dynamic array.']
            },
            {
              item: 'Linked list-based implementation:',
              subItems: ['Use a singly linked list or a doubly linked list to store the elements of the queue. Maintain two pointers, front and rear, to keep track of the start and end of the queue. When an element is enqueued, it is added to the rear of the linked list, and when an element is dequeued, it is removed from the front of the linked list. This implementation allows for dynamic resizing of the queue.']
            },
            {
              item: 'Circular array-based implementation:',
              subItems: ['Use a circular array to store the elements of the queue. Maintain two pointers, front and rear, to keep track of the start and end of the queue. When an element is enqueued, it is added to the rear of the circular array, taking care of wrapping around when the rear reaches the end of the array. When an element is dequeued, it is removed from the front of the circular array, again taking care of wrapping around when the front reaches the end of the array. This implementation allows efficient space utilization.']
            },
            {
              item: 'Doubly ended queue (Deque):',
              subItems: ['Use a doubly linked list or a circular array to implement a deque. A deque supports insertion and deletion of elements from both ends, unlike a standard queue. This allows for operations like enqueueing and dequeueing from both ends of the deque, making it more versatile.']
            },
            {
              item: 'Two stacks implementation:',
              subItems: ['Use two stacks to implement a queue. One stack is used for enqueue operations, and the other stack is used for dequeue operations. When an element is enqueued, it is pushed onto the enqueue stack. When an element is dequeued, if the dequeue stack is empty, all the elements from the enqueue stack are popped and pushed onto the dequeue stack, reversing their order. This ensures that the oldest element is at the top of the dequeue stack and can be popped to simulate a dequeue operation. This implementation provides a queue behavior using two stacks.']
            }
          ]}
        />
        <Example code="These are just a few examples of possible methods to implement a queue using different data structures. Each method has its own advantages and considerations, such as time complexity, space complexity, and flexibility. The choice of implementation depends on the specific requirements and constraints of the problem you are trying to solve." />
        <Heading level={2}>Stack data structure can be used to solve various problems in software engineering. Here are a few examples of problems where stack can be utilized:</Heading>
        <BulletPoints
          items={['Parentheses Matching: Use a stack to check if the parentheses in a given expression are balanced or not. This problem is commonly used to validate the syntax of programming language constructs.',
            ...
            'Browser History: Implementing a browser is back and forward functionality can be done using a stack. Each visited URL is pushed onto the stack, and going back involves popping the URL from the stack.'
          ]}
        />
        <Example code="These are just a few examples, and there are many more problems where stack data structure can be effectively used. The key is to understand the Last-In-First-Out (LIFO) nature of stacks and identify scenarios where maintaining a LIFO order is beneficial for solving the problem efficiently." />
        <HorizontalRule />
        <Heading level={2}>Now, let's test our understanding of stacks and queues with some thought-provoking questions. Get ready to delve into the intricacies of LIFO and FIFO, and see how well we grasp these concepts. Let's begin!</Heading>
        {questions.map((question, index) => (
          <Question
            key={index}
            title={question.title}
            description={question.description}
            detailsLink={question.detailsLink}
          />
        ))}
        <HorizontalRule />
        <Footer />
